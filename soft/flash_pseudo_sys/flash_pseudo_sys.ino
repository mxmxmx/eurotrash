...
String filename[500];
unsigned int file_position[500];
uint32_t file_adr[256];
char* nice_names[256];
char recovered_names[256][8];
uint8_t _EXT = 0;


uint8_t file_info; 
const uint8_t file_info_slot = 0xc;
...
bool verify(void)
{
    ...
    uint16_t files_nr = ((uint16_t)(buf2[0]) << 8) + buf2[1]; // extract file #
    uint8_t  page_offset = 1 + ((files_nr*file_info_slot + 0x2) >> 8); // page offset
...
void flash(void)
{
   ...
   fcnt = 0;
   pos =  0;
   page = file_info; // file info offset 
   ...
   while(1) {
    ...
    page = 0;
    uint16_t slot = 0;
    uint16_t file_info_bytes = file_info * PAGE;
    uint8_t pseudo_file_sys[file_info_bytes]; 
    Serial.println("  ");
    Serial.printf("Generating file info (%d page(s)):", file_info);
  
    memset(pseudo_file_sys, 0xff, file_info_bytes); 
    // file #  
...
bool extract(void)
{
    ...
    get_file_info(tmp_buf, files_nr); // we continue at byte 3
...
void get_file_info(uint8_t *fileinfo, uint8_t _files) {
...
void setup()
{
  ...
  file_info = 1 + ((fcnt*file_info_slot + 0x2) >> 8); // == # pages; 12 bytes: pos + name + fcnt;
  Serial.printf("\t\t\tPages consumed for file info: %d page(s)\r\n", file_info);
  fsize += file_info*PAGE;
